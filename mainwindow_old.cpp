...
#if defined(_NEOFACE)
#if defined(WIN32)
#include "../cmirisLib/win_neoface/NeoFacePro.h"
#elif defined(__linux__)
#include "../cmirisLib/linux_neoface/NeoFace.hpp"
#define NeoFacePro NeoFaceM
#endif

///////////////////////////////////////////////////////////////////
// NEC NeoFacePro Algorithm implementation
///////////////////////////////////////////////////////////////////
#define CMI_NEO_FACE_FEATURE_TEMPLATE_SIZE              5047

// Error return values in addition to values defined in cmiglobal.h
#define CMI_NEO_ERROR_LICENSE                   -2048
#define CMI_NEO_ERROR_INITAIALIZE               -2049
#define CMI_NEO_ERROR_NO_INPUT_IMAGES           -2050
#define CMI_NEO_ERROR_WRONG_TEMPLATE_SIZE       -2051
#define CMI_NEO_ERROR_LICENSE_2                 -2052
#define CMI_NEO_ERROR_MAKE_CFACEINFO            -2053
#define CMI_NEO_ERROR_CFACEINFO_FINDFACE        -2054
#define CMI_NEO_ERROR_MAKE_CFACEEATURE          -2055
#define CMI_NEO_ERROR_CFACEFEATURE_SERIALIZE    -2056
#define CMI_NEO_ERROR_MAKE_CVERIFIER            -2057
#define CMI_NEO_ERROR_FAIL_TO_COMPARE_TEMPLATES -2058

///////////////////////////////////////////////////////////////////
// End of NEC NeoFacePro Algorithm implementation
///////////////////////////////////////////////////////////////////
#endif
	...
	ui(new Ui::MainWindow)
{
    ...
    ////////////// INTERNAL USE ONLY /////////////////////////////////////////////
    connect(ui->pushButton_SaveDebugInfo, SIGNAL(clicked()), this, SLOT(saveDebugInfo()));
    ////////////// END OF INTERNAL USE ONLY /////////////////////////////////////
    ...
    iRet = NeoFacePro::Initialize();
    if(iRet == NFP_LICENSE_ERROR) qDebug() << "NeoFace License error";
    else if(iRet != NFP_SUCCESS) qDebug() << QString("Fail to initialize NeoFace. %1").arg(iRet);

    // Create CFaceInfo //
    NeoFacePro::CFaceInfo *faceInfo;
    try{
        m_faceInfo = (void *) new NeoFacePro::CFaceInfo();
        faceInfo = (NeoFacePro::CFaceInfo *) m_faceInfo;
    }catch(int err){
        if(err == NFP_NOT_INITIALILZE_ERROR) ERROR_OUT << "License Error";
        else qDebug() << "Unknown exception";
    }

    // Set the detection parameter.
#if defined(WIN32)
    //faceInfo->SetParamAlgorithm(NFP_ALGORITHM003F2EX);
    faceInfo->SetParamAlgorithm(NFP_ALGORITHM004);
#elif defined(__linux__)
    faceInfo->SetParamAlgorithm(NFP_ALGORITHM009F7);
#endif
    faceInfo->SetParamEyesRoll(15);
    faceInfo->SetParamReliability(0.5f);
    faceInfo->SetParamMaxFace(1);


    // Create CFeature
    m_feature = (void *) new NeoFacePro::CFaceFeature();
    NeoFacePro::CFaceFeature *feature = (NeoFacePro::CFaceFeature *) m_feature;

#if defined(WIN32)
                //feature->SetParamFeatureType(NFP_FEATURE_S14);
                feature->SetParamFeatureType(NFP_FEATURE_S15);
#elif defined(__linux__)
                feature->SetParamFeatureType(NFP_FEATURE_S16);
#endif

    // Create CVerify
    try{
        m_verify = (void *) new NeoFacePro::CVerifier();
    }catch(int err){
        if(err == NFP_NOT_INITIALILZE_ERROR) ERROR_OUT << CMI_NEO_ERROR_LICENSE;
        else qDebug() << CMI_NEO_ERROR_MAKE_CVERIFIER;
    }
...
MainWindow::~MainWindow()
{
    ...
    /////// NEOFACE ////////////////////////////////////
#if defined(_NEOFACE)
    if (m_faceInfo) delete m_faceInfo;
    if (m_feature) delete m_feature;
    if (m_verify) delete m_verify;

    // Terminate NeoFacePro
    int iRet;
    iRet = NeoFacePro::Terminate();
    if(iRet != NFP_SUCCESS)
        qDebug() << QString("Terminate error. %1, -6").arg(iRet);
#endif
    /////// NEOFACE ////////////////////////////////////
    ////////////// END OF INTERNAL USE ONLY /////////////////////////////////////
...
void MainWindow::open() {
	...
	case CMI_SUCCESS:
        ...
        ////////////// INTERNAL USE ONLY /////////////////////////////
#if defined(_NEOFACE)
        int retDB = ((MPICMain *)m_cmiHandle)->neoLoadDatabase(&m_database);
        if (retDB == -1){
            ui->statusBar->showMessage("Fail to load Enroll.db on cmirisLib");
            return;
        }
#endif
        ////////////// END OF INTERNAL USE ONLY //////////////////////
...
#if defined(__linux__)
        system(QString("aplay %1/recognized.wav").arg(m_curPath));
#else
...
void MainWindow::recog() {
    ...
    if (cmi_setInOutDepthOfField(m_cmiHandle, 35, 25) == CMI_SUCCESS) {
        m_inDepthOfField = 35;
        m_outDepthOfField = 25;
        m_minOperatingDistance = m_operatingRangeCenter - m_inDepthOfField;
        m_maxOperatingDistance = m_operatingRangeCenter + m_outDepthOfField;
        setupDistanceBar();
    }
    ...
    ////////////// INTERNAL USE ONLY /////////////////////////////
#if defined(_NEOFACE)
    // NeoFace Score Threshold Setting
    int retThSet = ((MPICMain *)m_cmiHandle)->neoSetScoreThreshold((float)ui->doubleSpinBox_NeoThreshold->value());
    if (retThSet == -1){
        ui->statusBar->showMessage("Fail to set NeoFace Score Threshold");
        return;
    }
    // NeoFace Verify Enable Setting
    bool setEnable;
    setEnable = ui->checkBox_NeoIsEnableFaceVerify->isChecked() ? true : false;
    bool retEnable = ((MPICMain *)m_cmiHandle)->neoSetEnableFaceVerify(setEnable);
    if (retEnable != setEnable){
        ui->statusBar->showMessage("Fail to set Enable NeoFace Verify");
        return;
    }
#endif
    ////////////// END OF INTERNAL USE ONLY //////////////////////
...
////////////// INTERNAL USE ONLY /////////////////////////////////////////////
#if defined(_NEOFACE)
    int iRet, width, height;
    unsigned char *faceImage;

    if(imageInfo->faceWidth == 720)
    {
        width = imageInfo->faceWidth/2;
        height = imageInfo->faceHeight/2;
        faceImage = new unsigned char[width*height];

        int i, j, k1, k2;
        for(i=0; i<height; i++){
            k1 = i*width;
            k2 = i*2*imageInfo->faceWidth;
            for(j=0; j<width; j++)
                faceImage[k1+j] = imageInfo->faceImage[k2+(j*2+1)];
		}
    }
    else if(imageInfo->faceWidth == 360)
    {
        width = imageInfo->faceWidth;
        height = imageInfo->faceHeight;
    ...
    NeoFacePro::CFaceInfo *faceInfo = (NeoFacePro::CFaceInfo *) m_faceInfo;
    faceInfo->SetParamEyesMaxWidth(width);
    faceInfo->SetParamEyesMinWidth(width/4);
    iRet = faceInfo->FindFace((BYTE*)faceImage, width, height);
    ...
    if(iRet != NFP_SUCCESS)
    {
        ...
        qDebug() << QString("cannot find face.");
        return;
    }

    NeoFacePro::CFaceFeature *enrol_feature = (NeoFacePro::CFaceFeature *) m_feature;

    iRet = enrol_feature->SetFeature(faceInfo);
    if(iRet != NFP_SUCCESS)
    {
        qDebug() << "make feature error. " << iRet;
        return;
    }

    void *faceFeatureTemplate;
    long templateSize;

    // Serialize the Compound feature.
    iRet = enrol_feature->Serialize(&faceFeatureTemplate, &templateSize, NFP_SERIALIZETYPE_COMPOUND);
    if(iRet != NFP_SUCCESS)
    {
...
////////////// INTERNAL USE ONLY /////////////////////////////////////////////
#if defined(_NEOFACE)
        qba = QByteArray((const char *)faceFeatureTemplate, templateSize);
        record.setFaceFeatureTemplate(qba);

        // Free serealized data.
        NeoFacePro::CFaceFeature::FreeSerializeData(faceFeatureTemplate);
...
#if defined(__linux__)
            system(QString("aplay %1/enrollCompleted.wav").arg(m_curPath));
#else
...
#if defined(__linux__)
        system(QString("aplay %1/notRecognized.wav").arg(m_curPath));
#else
...
#if defined(__linux__)
    system(QString("aplay %1/captureSound.wav").arg(m_curPath));
#else
...
void MainWindow::doIndicator(int indicatorValue) {
    if (indicatorValue == CMI_INDICATOR_COLOR_OFF)
...
#if defined(__linux__)
            system(QString("aplay %1/PleaseComeCloser.wav").arg(m_curPath));
#else
        ...
        if (m_isVocalCueEnabled) {
#if defined(__linux__)
            system(QString("aplay %1/PleaseMoveBack.wav").arg(m_curPath));
#else
...
void MainWindow::doMatchedFaceImage(CMI_IMAGE_INFO *imageInfo) {
    ...
    else ui->label_Name->setText(QString("Success : %1\n%2% Face Matched!").arg(record->name()).arg(imageInfo->rollAngleUncertainty));
...
void MainWindow::saveDebugInfo() {
    ((MPICMain *)m_cmiHandle)->saveDebugInfo();
	ui->statusBar->showMessage("Debug Image is saved.");
}
////////////// END OF INTERNAL USE ONLY //////////////////////
...
void MainWindow::displayLiveImages(CMI_IMAGE_INFO *imageInfo) {
    ...
    ////////////// INTERNAL USE ONLY /////////////////////////////
    MPICMain *mpicMain = (MPICMain *)m_cmiHandle;
    QString temp = QString("L %1, R %2, F %3").arg(mpicMain->m_leftTemp)
            .arg(mpicMain->m_rightTemp).arg(mpicMain->m_faceTemp);
    //ui->label_Temparature->setText(temp);
    ////////////// END OF INTERNAL USE ONLY //////////////////////
